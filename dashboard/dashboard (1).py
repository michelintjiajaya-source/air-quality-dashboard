...
with tab1a:
    ...
    # Insight dinamis - aman untuk jumlah stasiun berapapun
    sorted_stations = station_avg.sort_values(ascending=False)
    n_st = len(sorted_stations)
    worst_st = sorted_stations.index[0]
    best_st  = sorted_stations.index[-1]
    if n_st >= 3:
        insight_txt = (
            f"Stasiun dengan PM2.5 tertinggi: **{sorted_stations.index[0]}** ({sorted_stations.iloc[0]:.1f}), "
            f"**{sorted_stations.index[1]}** ({sorted_stations.iloc[1]:.1f}), "
            f"**{sorted_stations.index[2]}** ({sorted_stations.iloc[2]:.1f} ug/m3). "
            f"Terendah: **{best_st}** ({sorted_stations.iloc[-1]:.1f} ug/m3)."
        )
    elif n_st == 2:
        insight_txt = (
            f"Dari 2 stasiun yang dipilih, **{sorted_stations.index[0]}** ({sorted_stations.iloc[0]:.1f} ug/m3) "
            f"lebih tinggi dibanding **{sorted_stations.index[1]}** ({sorted_stations.iloc[1]:.1f} ug/m3)."
        )
    else:
        insight_txt = f"Stasiun **{worst_st}** memiliki rata-rata PM2.5 sebesar {sorted_stations.iloc[0]:.1f} ug/m3."
    st.info(insight_txt)
...
with tab1b:
    ...
    if len(avail_seasons) > 0:
        ...
        st.info(
            f"Musim **{worst_season}** memiliki rata-rata PM2.5 tertinggi "
            f"({season_avg[worst_season]:.1f} ug/m3), sedangkan **{best_season}** terendah "
            f"({season_avg[best_season]:.1f} ug/m3). "
            f"Konsentrasi memuncak pada pukul **{peak_hour:02d}.00** "
...
with tab1c:
    ...
    st.info(
        f"Bulan dengan rata-rata PM2.5 tertinggi: **{month_labels[max_month_idx-1]}**, "
        f"terendah: **{month_labels[min_month_idx-1]}**. "
...
with tab2a:
    ...
    st.info(
        f"Variabel dengan korelasi **negatif terkuat** terhadap PM2.5: "
        f"**{strongest_neg}** (r = {corr_pm25[strongest_neg]:.2f}). "
        f"Korelasi **positif terkuat**: **{strongest_pos}** (r = {corr_pm25[strongest_pos]:.2f})."
    )
...
with tab2b:
    ...
    low_wind  = dff_copy[dff_copy["wspm_cat"] == "0-1"]["PM2.5"].median()
    high_wind = dff_copy[dff_copy["wspm_cat"] == ">5"]["PM2.5"].median()
    if not np.isnan(low_wind) and not np.isnan(high_wind) and low_wind > 0:
        pct_drop = (low_wind - high_wind) / low_wind * 100
        st.info(
            f"Median PM2.5 pada angin lemah (0-1 m/s): **{low_wind:.0f} ug/m3**, "
            f"turun ke **{high_wind:.0f} ug/m3** pada angin kencang (>5 m/s), "
            f"penurunan sebesar **{pct_drop:.0f}%**."
        )
